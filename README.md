...
Each queue has an optional 24-hour upload cap. You can enable or disable it, choose the maximum number of accepted uploads, and manually reset the selected account's Reel Queue counter. When an enabled cap is reached, that account pauses for 24 hours from its newest accepted upload and then resumes automatically; other account profiles continue independently. This counter tracks uploads made by Reel Queue, not posts made manually outside the app, and resetting it does not change a platform's own limits.
...
- The next capped batch is prepared with two background converter workers. Prepared MP4s are cached across safe stops and restarts, so stopping before the chosen upload cap does not discard completed conversion work.
